...
export async function POST(req: Request) {
  try {
      ...
      tools: {
        get_available_slots: tool({
          ...
          parameters: z.object({
            specialty: z
              .string()
              .describe('Medical specialty, e.g. general, dermatology, orthopedics, mental health'),
            date: z
              .string()
              .optional()
          ...
          execute: async ({ specialty, date }) => {
            ...
            return date ? slots.filter((s) => s.datetime.startsWith(date)) : slots;
          },
        }),
        ...
        book_appointment: tool({
          ...
          parameters: z.object({
            slot_id: z.string().describe('The ID of the appointment slot the patient selected'),
            patient_name: z.string().describe('Full legal name of the patient'),
            patient_email: z.string().email().describe('Email address for booking confirmation'),
            specialty: z.string().describe('Medical specialty for this appointment'),
          }),
          execute: async ({ slot_id, patient_name, patient_email, specialty }) => {
            ...
            return {
              ...
              message: `Appointment confirmed for ${patient_name} with ${
                slot.provider
              } on ${new Date(slot.datetime).toLocaleString('en-US', {
                dateStyle: 'long',
                timeStyle: 'short',
        ...
        escalate_to_human: tool({
          ...
          parameters: z.object({
            reason: z
              .string()
              .describe('The specific emergency symptom or situation that triggered escalation'),
            severity: z
              .enum(['high', 'critical'])
              .describe(
    ...
    return new Response(
      JSON.stringify({ error: error?.message || error?.toString() || 'Unknown error' }),
      { status: 500, headers: { 'Content-Type': 'application/json' } }
    );
  }
}